...
func _process(_delta: float) -> void:
	...
	var mouse_position: Vector2 = get_global_mouse_position() - viewport.get_mouse_position() * 0.75# * 0.25 - viewport.get_camera().gloal_position# + viewport.size * 0.5# * (viewport.size.y / ProjectSettings.get("display/window/size/height"))
