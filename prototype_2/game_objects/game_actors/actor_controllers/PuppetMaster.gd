...
func _process(_delta: float):
	...
	if _animation_tree.can_act:
		var commands: Array = _puppeteer.get_input(object_of_interest, _hit_box, _game_actor.position, _current_segments, _path_progress)
...
func _update_path_progress(new_vector: Vector2):
	if pathfinding_target:
		_path_progress = int(max(_path_progress, _current_path.find(new_vector)))
