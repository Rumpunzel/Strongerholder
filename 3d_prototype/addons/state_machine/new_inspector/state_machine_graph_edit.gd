...
var state_machine: StateMachine2 = null setget set_state_machine
...
func _update_transitions(new_transitions: Array) -> void:
	...
	var states_list := state_machine.get_states_list()
	for i in states_list.size():
		var state: NodePath = states_list[i]
		var offset: Vector2 = state_machine._graph_offsets.get(state, Vector2(0.0, (i + 1) * _size.y))
		_add_state(state, offset)
	
	for i in new_transitions.size():
		var transition_item: TransitionItem = new_transitions[i]
		var offset: Vector2 = state_machine._graph_offsets.get(transition_item.resource_path, Vector2(_size.x, (i + 1) * _size.y))
...
func _add_transition(transition_item_resource: TransitionItem, offset: Vector2) -> TransitionItemGraphNode:
	if not state_machine._transitions.has(transition_item_resource):
		state_machine._transitions.append(transition_item_resource)
...
func _add_back_to_entry_point_node(offset: Vector2) -> CustomGraphNode:
	...
	new_graph_node.get_node("Label").text = state_machine.entry_state
...
func set_state_machine(new_state_machine: StateMachine2) -> void:
	state_machine = new_state_machine
	
	_update_transitions(state_machine._transitions)
	if state_machine.entry_state:
		var entry_node := _has_state(state_machine.entry_state)
...
func _on_transition_item_delete_requested(transition_item_graph_node: TransitionItemGraphNode) -> void:
	...
	state_machine._transitions.erase(transition_item_graph_node.transition_item_resource)
	_check_validity()
	...
	state_machine._graph_offsets.erase(transition_item_graph_node.transition_item_resource.resource_path)
